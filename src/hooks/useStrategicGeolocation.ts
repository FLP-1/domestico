...
export const useStrategicGeolocation = (
  ...
  const capture = useCallback(
    async (
      ...
      try {
        ...
        const errorMessage =
          error?.message || 'Erro desconhecido ao capturar geolocalização';

        return {
          success: false,
          error: errorMessage,
          permissionStatus:
            permissionStatus === 'granted' ? 'granted' : 'prompt',
        };
      }
    },
    [
   ...
   */
  const captureForCriticalAction = useCallback(
    async (
      actionName: string,
      ...
      if (!locationResult.success && locationResult.error) {
        console.warn(
          `⚠️ Geolocalização não disponível para ${actionName}:`,
