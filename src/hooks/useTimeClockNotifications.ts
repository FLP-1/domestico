...
import timeClockNotificationService, {
  TimeClockNotification,
} from '../services/timeClockNotificationService';
...
export const useTimeClockNotifications =
  (): UseTimeClockNotificationsReturn => {
    ...
    const refreshNotifications = useCallback(() => {
      const allNotifications = timeClockNotificationService
        .getNotificationsByType('pending_approval')
        .concat(
          timeClockNotificationService.getNotificationsByType(
            'overtime_request'
          )
        )
        .concat(
    ...
    const pendingApprovalCount = notifications.filter(
      n => n.tipo === 'pending_approval' && !n.lida
    ).length;
    const overtimeRequestCount = notifications.filter(
      n => n.tipo === 'overtime_request' && !n.lida
    ).length;
    const geolocationIssueCount = notifications.filter(
      n => n.tipo === 'geolocation_issue' && !n.lida
    ).length;
