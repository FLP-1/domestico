  ...
  const safeTheme = {
    colors: {
      ...
      status: {
        success: {
          background:
            typeof colors?.status?.success === 'object' &&
            colors?.status?.success &&
            'background' in colors.status.success
              ? String((colors.status.success as any).background)
              : typeof colors?.status?.success === 'string'
                ? colors.status.success
                : 'transparent',
          border:
            typeof colors?.status?.success === 'object' &&
            colors?.status?.success &&
            'border' in colors.status.success
          ...
          text:
            typeof colors?.status?.success === 'object' &&
            colors?.status?.success &&
            'text' in colors.status.success
              ? String((colors.status.success as any).text)
              : 'inherit',
        },
        warning: {
          background:
            typeof colors?.status?.warning === 'object' &&
            colors?.status?.warning &&
            'background' in colors.status.warning
              ? String((colors.status.warning as any).background)
              : typeof colors?.status?.warning === 'string'
                ? colors.status.warning
                : 'transparent',
          border:
            typeof colors?.status?.warning === 'object' &&
            colors?.status?.warning &&
            'border' in colors.status.warning
          ...
          text:
            typeof colors?.status?.warning === 'object' &&
            colors?.status?.warning &&
            'text' in colors.status.warning
              ? String((colors.status.warning as any).text)
              : 'inherit',
        },
        error: {
          background:
            typeof colors?.status?.error === 'object' &&
            colors?.status?.error &&
            'background' in colors.status.error
              ? String((colors.status.error as any).background)
              : typeof colors?.status?.error === 'string'
                ? colors.status.error
                : 'transparent',
          border:
            typeof colors?.status?.error === 'object' &&
            colors?.status?.error &&
            'border' in colors.status.error
          ...
          text:
            typeof colors?.status?.error === 'object' &&
            colors?.status?.error &&
            'text' in colors.status.error
              ? String((colors.status.error as any).text)
              : 'inherit',
        },
        info: {
          background:
            typeof colors?.status?.info === 'object' &&
            colors?.status?.info &&
            'background' in colors.status.info
              ? String((colors.status.info as any).background)
              : typeof colors?.status?.info === 'string'
                ? colors.status.info
                : 'transparent',
          border:
            typeof colors?.status?.info === 'object' &&
            colors?.status?.info &&
            'border' in colors.status.info
      ...
      // Superfície
      surface:
        colors?.surface || colors?.background?.secondary || 'transparent',
