...
async function updateUser(data: any, res: NextApiResponse) {
  try {
    const { usuarioId, cpf, email, nomeCompleto, dataNascimento, telefone } =
      data;
    ...
    const validation = await ValidationService.validateUserUpdate(
      usuarioId,
      cpf,
      email
    );
...
async function removeUserFromGroup(data: any, res: NextApiResponse) {
  try {
    ...
    const validation = await ValidationService.validateUserGroupRemoval(
      usuarioId,
      grupoId
    );
    if (!validation.isValid) {
