    ...
    if (response.ok) {
      ...
      if (data.address) {
        ...
        if (addr.road || addr.street) {
          ...
          const houseNumber =
            addr.house_number || addr.houseNumber || addr.housenumber;
          if (houseNumber) {
    ...
    if (bigDataResponse.ok) {
      ...
      if (data.locality && data.principalSubdivision) {
        ...
        if (data.localityInfo && data.localityInfo.administrative) {
          ...
          const specificInfo = data.localityInfo.administrative.find(
            (a: any) =>
              a.order >= 7 &&
              !a.name.includes('Região') &&
              !a.name.includes('Metropolitana') &&
              !a.name.includes('São Paulo') &&
              a.name.length > 5
          );
        ...
        if (
          data.countryName &&
          data.countryName !== data.principalSubdivision
        ) {
