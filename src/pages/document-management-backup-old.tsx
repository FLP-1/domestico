...
const DocumentInfoContainer = styled.div<{ $theme?: Theme }>`
  padding: 1rem;
  background: ${props => {
    const surface = props.$theme?.colors?.surface;
    const surfaceColor =
...
const DocumentInfoTitle = styled.h4<{ $theme?: Theme }>`
  margin: 0 0 0.5rem 0;
  color: ${props => {
    const text = props.$theme?.colors?.text;
    return (
...
export default function DocumentManagement() {
  ...
  const documentActions: DataListAction[] = [
    ...
    },
    {
      ...
      variant: 'secondary',
      onClick: (item: DataListItem) => setSelectedDocumentIdForChat(item.id), // ✅ NOVO: Abrir comunicação contextual
    },
    {
      icon: '❌',
