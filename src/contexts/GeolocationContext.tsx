...
export const GeolocationProvider = ({ children }: { children: ReactNode }) => {
  ...
  const updateLastLocationIfBetter = useCallback(
    (location: GeolocationData) => {
      ...
      const Δλ =
        ((location.longitude - lastLocation.longitude) * Math.PI) / 180;
      const a =
      ...
      if (distancia > 100) {
        ...
        if (lastLocation.accuracy < 200 && location.accuracy > 1000) {
          logger.geo(
            '📍 Ignorando coordenada ruim após ter recebido coordenada precisa:',
            {
              distancia: Math.round(distancia),
              antiga: {
                lat: lastLocation.latitude,
                lon: lastLocation.longitude,
                accuracy: Math.round(lastLocation.accuracy),
              },
              nova: {
                lat: location.latitude,
                lon: location.longitude,
        ...
        if (location.accuracy < lastLocation.accuracy) {
          logger.geo(
            '📍 Localização mudou significativamente (>100m) e nova é MELHOR, atualizando:',
            {
              distancia: Math.round(distancia),
              antiga: {
                lat: lastLocation.latitude,
          ...
          logger.geo(
            '📍 Ignorando coordenada menos precisa após mudança de local:',
            {
              distancia: Math.round(distancia),
              antiga: {
                lat: lastLocation.latitude,
      ...
      if (isNewer && !isMoreAccurate) {
        ...
        if (location.accuracy <= lastLocation.accuracy * 2) {
          logger.geo(
            '📍 Localização mais recente (não muito pior), atualizando:',
            {
              antiga: {
                lat: lastLocation.latitude,
                lon: lastLocation.longitude,
      ...
      if (!isNewer && isMoreAccurate) {
        logger.geo(
          '📍 Localização mais precisa (mesmo que mais antiga), atualizando:',
          {
            antiga: {
              lat: lastLocation.latitude,
              lon: lastLocation.longitude,
