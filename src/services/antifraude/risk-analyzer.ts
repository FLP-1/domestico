...
async function analisarGeolocalizacao(
  ...
  try {
    ...
    const velocidadeNecessaria =
      distanciaKm / Math.max(tempoDecorridoHoras, 0.001);
...
export async function analisarRisco(
  ...
  try {
    ...
    const scoreIP = analiseIP.score * 0.3;
    const scoreGeolocalizacao = analiseGeo.score * 0.2;
    const scoreComportamento = analiseComp.score * 0.2;
    ...
    if (analiseFingerprint.bloqueado)
      sinaisAlerta.push('Dispositivo bloqueado');
    ...
    if (analiseGeo.velocidadeImpossivel)
      sinaisAlerta.push('Velocidade de deslocamento impossível');
    if (analiseComp.botDetectado)
      sinaisAlerta.push('Comportamento de bot detectado');
