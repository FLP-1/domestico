...
export const LoadingOverlay = styled.div<{ $isLoading: boolean; $theme?: any }>`
  position: relative;
...
export const InfoMessage = styled.div<{
  ...
  background: ${props => {
    if (props.$variant) {
      return (
        props.$theme?.colors?.status?.[props.$variant]?.background ||
  ...
  color: ${props => {
    if (props.$variant) {
      return (
        props.$theme?.colors?.status?.[props.$variant]?.text ||
  ...
  border: 1px solid
    ${props => {
      if (props.$variant) {
        return (
          props.$theme?.colors?.status?.[props.$variant]?.border ||
