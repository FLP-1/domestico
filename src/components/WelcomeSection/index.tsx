...
const StatusBadge = styled.span<{ $variant?: 'ok' | 'warn' | 'pending'; $theme?: any }>`
  ...
  font-weight: 600;
  color: ${({ $variant, $theme }) => {
    const baseColor = $variant === 'warn' || $variant === 'pending' 
...
const GroupName = styled.span<{ $color?: string; $theme?: any }>`
  color: ${props => props.$color || props.$theme?.colors?.primary || defaultColors.primary};
  font-weight: 500;
  font-size: 0.85rem;
  line-height: 1.2;
`;
...
const NotificationButton = styled.button<{ $theme?: any }>`
  ...
  border: 1px solid ${props => props.$theme?.colors?.primary ? addOpacity(props.$theme.colors.primary, 0.3) : addOpacity(defaultColors.primary, 0.3)};
  border-radius: 12px;
  padding: 0.75rem;
  ...
  &:hover {
    background: ${props => props.$theme?.colors?.primary ? addOpacity(props.$theme.colors.primary, 0.3) : addOpacity(defaultColors.primary, 0.3)};
            ...
            <LocationInfo>
              {(() => {
                ...
                if (!currentLocation) {
                  return (
                    <LocationMessage>
                      {isCapturing ? 'Capturando localização atual...' : 'Não foi possível identificar a localização'}
                    </LocationMessage>
                  );
                }
