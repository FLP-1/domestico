...
export class BehaviorTracker {
  ...
  private calcularVelocidadeDigitacao(): { media: number; desvio: number } {
    ...
    const soma = this.intervalosTeclas.reduce(
  ...
  public gerarRelatorio(): DadosComportamentais {
    ...
    const tempoEntreAcoes = this.eventos
      .slice(0, 100)
      .map((e: any, i: any) =>
        i > 0 ? e.timestamp - this.eventos[i - 1].timestamp : 0
      )
      .filter(t => t > 0);

    const sequenciaEventos = this.eventos.slice(-50).map(e => e.tipo);

    const tempoSessaoSegundos = Math.floor(
      (Date.now() - this.inicioSessao) / 1000
    );
    ...
    return {
      ...
      velocidadeMouseMed:
        this.movimentosMouse / Math.max(tempoSessaoSegundos, 1),
      padraoCliques,
      tempoEntreAcoes,
      sequenciaEventos,
