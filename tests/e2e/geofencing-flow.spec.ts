...
test.describe('Fluxo Completo de Geofencing', () => {
  ...
  test('deve criar local de trabalho', async ({ page }) => {
    ...
    if (await createButton.isVisible()) {
      ...
      const addressInput = page
        .locator(
          'input[name="endereco"], input[name="address"], textarea[name="endereco"]'
        )
        .first();
  ...
  test('deve exibir lista de locais', async ({ page }) => {
    ...
    const locationsList = page
      .locator(
        '[data-testid="locations-list"], .locations-list, table, [class*="location"]'
      )
      .first();
    const emptyState = page.locator('text=/nenhum local|sem locais/i').first();

    const hasList = await locationsList
  ...
  test('deve exibir auditoria de geofencing', async ({ page }) => {
    ...
    const auditTable = page
      .locator('table, [data-testid="audit-list"], .audit-list')
      .first();
    const emptyState = page
      .locator('text=/nenhum registro|sem registros/i')
      .first();
