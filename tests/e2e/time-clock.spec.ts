...
test.describe('Registro de Ponto', () => {
  ...
  test('deve exibir interface de registro de ponto', async ({ page }) => {
    ...
    const hasClockButton = await page.locator('button:has-text("Entrar"), button:has-text("Sair"), [data-testid="clock-button"]').isVisible().catch(() => false);
    const hasTimeDisplay = await page.locator('text=/hora|horário|time/i, [data-testid="time-display"]').isVisible().catch(() => false);
  ...
  test('deve permitir registro de entrada', async ({ page }) => {
    ...
    const clockButton = page.locator('button:has-text("Entrar"), button:has-text("Registrar"), [data-testid="clock-button"]').first();
    ...
    if (isVisible) {
      ...
      const hasFeedback = await page.locator('text=/sucesso|registrado|confirmado/i, [role="alert"]').isVisible().catch(() => false);
  ...
  test('deve exibir histórico de registros', async ({ page }) => {
    ...
    const hasHistory = await page.locator('text=/histórico|registros|history/i, [data-testid="history"]').isVisible().catch(() => false);
