...
test.describe('Fluxo Completo de Criação de Tarefas', () => {
  ...
  test('deve criar tarefa com sucesso', async ({ page }) => {
    ...
    const createButton = page
      .locator(
        'button:has-text("Criar"), button:has-text("Nova Tarefa"), button:has-text("Adicionar")'
      )
      .first();
    ...
    if (await createButton.isVisible()) {
      ...
      const titleInput = page
        .locator(
          'input[name="titulo"], input[name="title"], input[placeholder*="título"], input[placeholder*="title"]'
        )
        .first();
      const descriptionInput = page
        .locator(
          'textarea[name="descricao"], textarea[name="description"], textarea[placeholder*="descrição"]'
        )
        .first();
      ...
      if (await titleInput.isVisible({ timeout: 3000 })) {
        ...
        const saveButton = page
          .locator(
            'button:has-text("Salvar"), button:has-text("Criar"), button[type="submit"]'
          )
          .first();
  ...
  test('deve exibir lista de tarefas', async ({ page }) => {
    ...
    const taskList = page
      .locator('[data-testid="task-list"], .task-list, table, [class*="task"]')
      .first();
    const emptyState = page
      .locator('text=/nenhuma tarefa|sem tarefas/i')
      .first();

    const hasList = await taskList
